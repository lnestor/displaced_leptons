#!/usr/bin/env python3
"""
Compare the muon eta-phi veto between coffea and CMSSW DisplacedSUSY.

CMSSW stage semantics: StageN tree = events PASSING cut N.
  Stage08MuEtaTreeMaker        -> events entering the muon eta-phi veto (passed mu eta)
  Stage09MuEtaPhiVetoTreeMaker -> events passing the muon eta-phi veto

Coffea semantics: stage_events[label] = events passing that cut (cumulative).

Disagreements (after removing known reasons):
  pass CMSSW veto, fail coffea veto: in Stage09 but not in coffea mu etaphi set
  pass coffea veto, fail CMSSW veto: in coffea mu etaphi set but not in Stage09
"""
...
ETAPHI_LABEL = "$>=1$ $\\mu$ passing $\\eta$-$\\phi$ veto"
...
# -- Load CMSSW trees ----------------------------------------------------------
# Stage08 = events entering the muon eta-phi veto (passed mu eta)
# Stage09 = events passing the muon eta-phi veto
...
cmssw_entering = to_event_set(s08["eventvariable_run"], s08["eventvariable_ls"], s08["eventvariable_event"])
cmssw_passed   = to_event_set(s09["eventvariable_run"], s09["eventvariable_ls"], s09["eventvariable_event"])
...
print(f"CMSSW entering mu etaphi veto (Stage08): {len(cmssw_entering)}")
print(f"CMSSW passing mu etaphi veto  (Stage09): {len(cmssw_passed)}")
print(f"Coffea passing mu etaphi veto:           {len(coffea_passed)}")
...
print(f"--- Pass CMSSW mu etaphi, fail coffea (showing {min(N_PRINT, len(pass_cmssw_fail_coffea))}) ---")
for run, lumi, event in pass_cmssw_fail_coffea[:N_PRINT]:
    i = s09_idx[(run, lumi, event)]
    print(f"  run={run:>7}  lumi={lumi:>5}  event={event:>12}"
          f"  cmssw_mu_eta={s09['muon_etaMuon0'][i]:+.3f}"
          f"  cmssw_mu_pt={s09['muon_ptMuon0'][i]:.1f}")

print()
print(f"--- Pass coffea mu etaphi, fail CMSSW (showing {min(N_PRINT, len(pass_coffea_fail_cmssw))}) ---")
for run, lumi, event in pass_coffea_fail_cmssw[:N_PRINT]:
    i = coffea_idx[(run, lumi, event)]
    cmssw_str = ""
    if (run, lumi, event) in s08_idx:
        j = s08_idx[(run, lumi, event)]
        cmssw_str = (f"  cmssw_mu_eta={s08['muon_etaMuon0'][j]:+.3f}"
                     f"  cmssw_mu_pt={s08['muon_ptMuon0'][j]:.1f}")
    else:
        cmssw_str = "  (not in CMSSW Stage08 -- failed mu eta cut)"
    print(f"  run={run:>7}  lumi={lumi:>5}  event={event:>12}"
