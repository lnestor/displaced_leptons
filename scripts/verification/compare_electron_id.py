#!/usr/bin/env python3
"""
Compare the electron tight ID cut between coffea and CMSSW DisplacedSUSY.

CMSSW stage semantics: StageN tree = events PASSING cut N.
  Stage05ElePtTreeMaker  -> events entering the ID cut (passed pt)
  Stage06EleIDTreeMaker  -> events passing the ID cut

Coffea semantics: stage_events[label] = events passing that cut (cumulative).

Disagreements (after removing known reasons):
  pass CMSSW ID, fail coffea ID: in Stage06 but not in coffea ID set
  pass coffea ID, fail CMSSW ID: in coffea ID set but not in Stage06
"""
...
# -- Load CMSSW trees ----------------------------------------------------------
# Stage05 = events entering the ID cut (passed pt)
# Stage06 = events passing the ID cut
...
cmssw_entering = to_event_set(s05["eventvariable_run"], s05["eventvariable_ls"], s05["eventvariable_event"])
cmssw_passed   = to_event_set(s06["eventvariable_run"], s06["eventvariable_ls"], s06["eventvariable_event"])
...
print(f"CMSSW entering ID cut (Stage05): {len(cmssw_entering)}")
print(f"CMSSW passing ID cut  (Stage06): {len(cmssw_passed)}")
print(f"Coffea passing ID cut:           {len(coffea_passed)}")
...
print(f"--- Pass CMSSW ID, fail coffea ID (showing {min(N_PRINT, len(pass_cmssw_fail_coffea))}) ---")
for run, lumi, event in pass_cmssw_fail_coffea[:N_PRINT]:
    i = s06_idx[(run, lumi, event)]
    print(f"  run={run:>7}  lumi={lumi:>5}  event={event:>12}"
          f"  cmssw_ele_eta={s06['electron_etaElectron0'][i]:+.3f}"
          f"  cmssw_ele_pt={s06['electron_ptElectron0'][i]:.1f}")

print()
print(f"--- Pass coffea ID, fail CMSSW ID (showing {min(N_PRINT, len(pass_coffea_fail_cmssw))}) ---")
for run, lumi, event in pass_coffea_fail_cmssw[:N_PRINT]:
    i = coffea_idx[(run, lumi, event)]
    cmssw_str = ""
    if (run, lumi, event) in s05_idx:
        j = s05_idx[(run, lumi, event)]
        cmssw_str = (f"  cmssw_ele_eta={s05['electron_etaElectron0'][j]:+.3f}"
                     f"  cmssw_ele_pt={s05['electron_ptElectron0'][j]:.1f}")
    else:
        cmssw_str = "  (not in CMSSW Stage05 -- failed pt cut)"
    print(f"  run={run:>7}  lumi={lumi:>5}  event={event:>12}"
